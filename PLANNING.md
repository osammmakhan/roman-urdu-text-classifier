...
This is a scoped personal project built to demonstrate full-stack ownership (DB + API + frontend) and AI-agent-assisted development using OpenCode, not a production system.
...
- **Hosting:** TBD — decided after the app is working locally (Hosted on Render now)
...
## Build approach
- Built using OpenCode as the primary AI coding tool
...
- Hosting choice (backend + frontend) — decide after core app works locally (Hosted on Render)
...
- Add unit tests for classification service (Added)
- Verify working Groq model on console.groq.com (Verified)
...
- Groq model: `openai/gpt-oss-20b` (configured in backend/app/config.py)